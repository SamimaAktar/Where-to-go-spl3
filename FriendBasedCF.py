...
class FriendBasedCF(object):
    ...
    def predict_user(self,friends,check_ins,j):
        social_proximity = []
        friends = set(friends)
        u_check_in_neighbors = set(check_ins.nonzero()[0])
        for fid in friends:
            f_social_neighbors = self.social_[fid]
            jaccard_friend = (1.0 * len(friends.intersection(f_social_neighbors)) /
                              len(friends.union(f_social_neighbors)))

            f_check_in_neighbors = self.check_in_matrix[fid, :].nonzero()[0]
            tot = len(u_check_in_neighbors.union(f_check_in_neighbors))
            if tot > 0:
                jaccard_check_in = 1.0 * len(u_check_in_neighbors.intersection(f_check_in_neighbors)) / tot
            else:
